...
class Jugador(Thread):
    ...
    def intentar_capturar_bandera(self) -> None:
        ...
        if not self.lock_bandera.acquire(False):
            return

        if self.bandera.nombre_dueño is None:
    ...
    def correr_segunda_mitad(self) -> bool | None:
        self.lock_carrera.acquire()

        if self.senal_fin.is_set():
            self._correr = False
            return False

        if self.posicion == 100 and self.tiene_bandera:
            self.senal_fin.set()
            self._correr = False
            return True

        if not self.tiene_bandera:
    ...
    def run(self) -> None:
        ...
        if not self.senal_inicio.is_set():
