...
from servidor.banco import BancoEncoder, BancoDecoder, Transaccion
...
class DCConsumo:
    ...
    def hacer_transferencia(
        self, nombre_banco: str, destinatario: dict, monto: int
    ) -> None:
        """
        ...
        if (
            None in (nombre_banco, monto, destinatario)
            or "nombre" not in destinatario
            or "n_cuenta" not in destinatario
        ):
            raise TypeError("Faltan argumentos")
        ...
        self.agregar_gasto(
            nombre_banco, f"Transferencia {dest_nombre} ({dest_cuenta})", monto
        )
