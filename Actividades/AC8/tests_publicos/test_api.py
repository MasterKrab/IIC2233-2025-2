...
class VerificarApi(unittest.TestCase):
    ...
    def test_obtener_transacciones_cantidad(self):
        ...
        with self.app.test_client() as client:
            response = client.get(
                "/obtener_transacciones/PepaBank", query_string={"cantidad": cantidad}
            )
    ...
    def test_obtener_transacciones_inexistente(self):
        ...
        with self.app.test_client() as client:
            ...
            self.assertEqual(response.status, "404 NOT FOUND")
            self.assertCountEqual(response.get_json(), self.saldo["NotBank"])
        ...
        with self.app.test_client() as client:
            ...
            with patch("servidor.main.dcconsumo.agregar_gasto") as ag:
                response = client.post(
                    f"/agregar_gasto/{banco}",
                    query_string={"titulo": titulo, "monto": monto},
                )
        ...
        with self.app.test_client() as client:
            ...
            with patch("servidor.main.dcconsumo.ajustar_saldo") as ag:
                response = client.patch(
                    f"/ajustar_saldo/{banco}", query_string={"saldo": saldo}
                )
    ...
    def test_hacer_transferencia(self):
        ...
        with self.app.test_client() as client:
            ...
            with patch("servidor.main.dcconsumo.hacer_transferencia") as ag:
                response = client.post(
                    "/hacer_transferencia", json=data, headers=header
                )

                self.assertEqual(response.status, "200 OK")
                self.assertEqual(
                    response.get_json(), {"msg": "Transferencia realizada"}
                )
                ag.assert_called_once_with(
                    banco, {"nombre": dest_nombre, "n_cuenta": dest}, monto
                )
    ...
    def test_hacer_transferencia_error_404(self):
        ...
        with self.app.test_client() as client:
            with patch(
                "servidor.main.dcconsumo.hacer_transferencia",
